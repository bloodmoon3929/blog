...
- 승인 검사(Acceptance Test, 인수 테스트)
	- 하나의 이터레이션 안에서 계획된 릴리즈 단위의 부분 완료 제품이 구형되면 수행하는 테스트이다.
...
- 소규모 릴리즈(Small Release)
	...
	- 계획된 릴리즈 기간 동안 진행된 이터레이션이 모두 완료되면 고객에 인한 최종 테스트를 수행한 후 릴리즈, 즉 최종 결과물을 고객에게 전달한다.
...
| Test-Driven-Development(테스트 주도 개발)              | - 개발자가 실제 코드를 작성하기 전에 테스트 케이스를 먼저 작성하므로 자신이 무엇을 해야할지 정확히 파악합니다.<br>- 테스트가 지속적으로 진행될 수 있도록 자동화된 테스팅 도구(구조 프레임워크)를 사용합니다. |
| Whole Team(전체 팀)                                | 개발에 참여하는 모든 구성원(고객 포함)들은 각자의 역할이 있고 그 역할에 대한 책임을 가져야 합니다.                                                               |
| Continuous Intergration(계속적인 통합)                | 모듈 단위로 나눠서 개발된 코드들은 하나의 작업이 마무리될 때마다 지속적인 통합됩니다                                                                         |
| Design Improvement(디자인 개선) 또는 Refactoring(리팩토링) | 프로그램 기능의 변경 없이, 단순한 유연성 강화 등을 통해 시스템을 재구성합니다.                                                                           |
| Small Release(소규모 릴리즈)                          | 릴리즈 기간을 짧게 반복함으로써 고객의 요구 변화에 신속히 대응할 수 있습니다.                                                                            |
