...
- E-R 모델에서는 데이터를 [[데이터 모델의 구성 요소|개체(Entity), 관계(Relationship), 속성(Attribute)]]으로 묘사한다.
- E-R 모델은 특정 DBMS를 고려한 것은 아니다.
- E-R 다이어그램으로 표현하며, [[데이터 모델의 구성 요소|1:1, 1:N, N:M]] 등의 관계 유형을 제한 없이 나타낼 수 있다.
- 최초에는 개체, 관계, 속성과 같은 개념들로 구성되었으나 나중에는 일반화 계층과 같은 복잡한 개념들이 첨가되어 확장된 모델로 발전했다.