...
이 때, a정점에 대해 b, d, f, g라는 인접한 정점이 있었고, b의 정점을 방문할 때, 인접한 정점인 c라는 정점은 d, f, g뒤에 위치해야 함 이를 위해 [[queue]]를 사용하면 편함
...
```
위의 코드의 입력은 다음과 같다.
![첨부파일/algrithm-graph.png](/src/site/img/user/%EC%B2%A8%EB%B6%80%ED%8C%8C%EC%9D%BC/algrithm-graph.png)

모두 정확히 동작함을 알 수 있다.

이 때 시간 복잡도는 인접 리스트로 구현된 경우 $O(|V|+|E|)$, 그리고 인접 형렬로 구현했을 경우 $O(|V|^2)$입니다.

[[깊이 우선 탐색]]과의 다르게 너비 우선 탐색은 그래프에서는 단 한 가지 역할을 하는 데, 최단 경로 문제를 풀 때만 사용한다.
